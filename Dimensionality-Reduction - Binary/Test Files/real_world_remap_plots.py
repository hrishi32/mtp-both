...
def get_all_errors(input_file, n_pairs, compensation1, compensation2):
    ...
    while count < n_pairs-1:
        ...
        batch_error_c, batch_time_c, batch_inner_product1_c,batch_inner_product2_c,_,_ = get_remap_results(Input_dimension = N,Output_dimension = M,array1=norm_arr_1,array2=norm_arr_2,mapping_scheme=8)
        ...
        if count%100 == 0 or count == n_pairs-2:
            np.save('/home/b16032/MTP/Dimensionality-Reduction/Test Files/Outputs/real_'+dataset+'_'+str(count)+'_'+time.ctime()+'.npy', [ np.array(avg_batch_error_a)/count, np.array(avg_batch_error_b)/count, np.array(avg_batch_error_c)/count ])
...
def main():
    ...
    #plt.show()
    fig.savefig('/home/b16032/MTP/Dimensionality-Reduction/Test Files/Plots/All_Datasets_1000_'+time.ctime()+'.png', orientation = 'landscape')

...
"""
    ...
    *   It then computes the affected output array and their cosine similarities. 
...
def get_all_errors(input_file, n_pairs, compensation1, compensation2):
    ...
    while count < n_pairs-1:
        ...
        if count%100 == 0 or count == n_pairs-2:
            np.save('Outputs/sample00120_real_'+dataset+'_'+str(count)+'_'+'.npy', [ np.array(avg_batch_error_a)/count, np.array(avg_batch_error_b)/count, np.array(avg_batch_error_c)/count ])
...
def main():
    ...
    #plt.show()
    fig.savefig('Plots/sample_00120_All_Datasets_1000_'+'.png', orientation = 'landscape')

...
"""
    ...
    *                 batch_jaccard_similarity1: Array of real numbers (Values of jaccard similarity of input arrays)
    *                 batch_jaccard_similarity2: Array of real numbers (Values of predicted jaccard similarity)
    ...
    *   It then computes the affected output array and their jaccard similarities. 
...
def get_all_errors(input_file, n_pairs, compensation1, compensation2):
    ...
    while count < n_pairs-1:
        ...
        batch_error_a, batch_time_a, batch_jaccard_similarity1_a,batch_jaccard_similarity2_a,_,_ = get_feature_insertion_results(Input_dimension = N,Output_dimension = M,default_maps=maps,array1=norm_arr_1,array2=norm_arr_2,mapping_scheme=5,max_value=alpha)

        batch_error_b, batch_time_b,batch_jaccard_similarity1_b,batch_jaccard_similarity2_b,_,_ = get_feature_insertion_results(Input_dimension = N,Output_dimension = M,default_maps=maps,array1=norm_arr_1,array2=norm_arr_2,mapping_scheme=6,max_value=alpha)
        ...
        if count%50 == 0 or count == n_pairs-2:
            np.save('Outputs/insertion/sample00030_testing_'+dataset+'_'+str(count)+'.npy', [ (np.array(avg_batch_error_a)/count, np.array(batch_time_a)/count), (np.array(avg_batch_error_b)/count, np.array(batch_time_b)/count), (np.array(avg_batch_error_c)/count, np.array(batch_time_c)/count) ])
...
def main():
    ...
    fig.savefig('Plots/sample00030_Insertion_1000.png', orientation = 'landscape')

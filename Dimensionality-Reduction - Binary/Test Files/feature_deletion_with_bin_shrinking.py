...
def get_all_errors(input_file, n_pairs, compensation1, compensation2):
    ...
    while count < n_pairs-1:
        ...
        batch_error_b, batch_time_b,batch_inner_product1_b,batch_inner_product2_b,_,_ = get_feature_deletion_results(Input_dimension = N,Output_dimension = M, default_maps=maps,array1=norm_arr_1,array2=norm_arr_2,mapping_scheme=9,max_value=alpha)
        ...
        if count%100 == 0 or count == n_pairs-2:
            np.save('Outputs/sample0002_real_'+dataset+'_'+str(count)+'_'+'.npy', [ np.array(avg_batch_error_a)/count, np.array(avg_batch_error_b)/count, np.array(avg_batch_error_c)/count ])
...
def main():
    ...
    fig.savefig('Plots/sample_0002_All_Datasets_1000_'+'.png', orientation = 'landscape')

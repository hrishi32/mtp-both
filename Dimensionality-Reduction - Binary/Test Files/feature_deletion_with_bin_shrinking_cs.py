...
"""
    ...
    *
    * Return Value  : batch_error: Array of real numbers (Error in original and predicted cosine similarity)
    *                 batch_time: Array of real numbers (Time taken)
    *                 batch_cosine_similarity1: Array of real numbers (Values of cosine similarity of input arrays)
    *                 batch_cosine_similarity2: Array of real numbers (Values of predicted cosine similarity)
    *                 array1: Array of binary numbers (Compressed array of array1)
    *                 array2: Array of binary numbers (Compressed array of array2)
    *
    * Description:
    *
    *   This function delets the numbers to given input array. Deletion with bin shrinking is used.
    *   It then computes the affected output array and their cosine similaritys. 
...
def get_all_errors(input_file, n_pairs, compensation1, compensation2):
    ...
    while count < n_pairs-1:
        ...
        batch_error_b, batch_time_b,batch_cosine_similarity1_b,batch_cosine_similarity2_b,_,_ = get_feature_deletion_results(Input_dimension = N,Output_dimension = M, default_maps=maps,array1=norm_arr_1,array2=norm_arr_2,mapping_scheme=9,max_value=alpha)
        ...
        if count%100 == 0 or count == n_pairs-2:
            np.save('Outputs/sample00110_real_'+dataset+'_'+str(count)+'_'+'.npy', [ np.array(avg_batch_error_a)/count, np.array(avg_batch_error_b)/count, np.array(avg_batch_error_c)/count ])
...
def main():
    ...
    for x, y in files.items():
        ...
        ax[1][it].plot(range(len(avg_batch_time_a)), cumulate(avg_batch_time_a), label="No Compensation", linestyle='--')
        ax[1][it].plot(range(len(avg_batch_time_b)), cumulate(avg_batch_time_b), label="Our Method", linewidth=3)
        ax[1][it].plot(range(len(avg_batch_time_c)), cumulate(avg_batch_time_c), label="Remap")
    ...
    #plt.show()
    fig.savefig('Plots/sample_00110_All_Datasets_1000_'+'.png', orientation = 'landscape')

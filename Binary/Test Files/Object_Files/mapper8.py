...
class mapper:
    def __init__(self, input_dim = 50, out_dim = 15):
        ...
        self.new_input_dimension = 0
        self.new_output_dimension = 0
    ...
    def insert_feature(self, position=0, remap = False):
        if remap == True:
            self.new_input_dimension = self.input_dimension + 1
            self.new_output_dimension = self.output_dimension + 1

            self.new_map = np.zeros(self.output_dimension, dtype=int)
            for i in range(self.output_dimension):
                alpha = random.randint(0,self.new_output_dimension-1)
                self.new_map[i] = alpha

            beta = random.randint(0, self.new_output_dimension-1)
            self.map = np.insert(self.map, position, beta,axis=0)
        else:
    ...
    def new_batch_insert_feature(self, no_of_features, no_of_bins, remap):
        if remap == True:
            self.new_input_dimension = self.input_dimension + no_of_features
            self.new_output_dimension = self.output_dimension + no_of_bins
            # print(self.new_output_dimension)
            # self.new_map = np.zeros(self.output_dimension, dtype=int)
            self.new_map = np.random.randint(self.new_output_dimension, size=self.output_dimension)
            # for i in range(self.output_dimension):
            #     alpha = random.randint(0,self.new_output_dimension-1)
            #     self.new_map[i] = alpha
            ...
            # beta = np.zeros(no_of_features,dtype=int)
            # self.feature_counter = []
            # for i in range(out_dim):
            #     self.feature_counter.append([])
            beta = np.random.randint(self.new_output_dimension, size = no_of_features)

            # for i in range(no_of_features):
            #     alpha = random.randint(0,self.new_input_dimension-1)
            #     beta[i] = alpha

            self.map = np.concatenate((self.map, beta), axis=0)


            # beta = random.randint(0, self.new_output_dimension-1)
            # self.map = np.insert(self.map, self.map.size-1, beta,axis=0)
        else:
            self.input_dimension +=no_of_features
            beta = np.zeros(no_of_features,dtype=int)
    ...
    def batch_insert_fearures(self, features):
        self.new_input_dimension = self.input_dimension+no_of_features
    ...
    def dimension_reduction(self, input_array):
        ...
        if self.new_output_dimension == 0:

            output_array = np.zeros(self.output_dimension, dtype=int)
            ...
            output_array = np.zeros(self.output_dimension, dtype=int)
            new_output_array = np.zeros(self.new_output_dimension, dtype=int)
            for i in range(self.input_dimension):
                output_array[self.map[i]] |= input_array[i]

            for i in range(self.output_dimension):
                new_output_array[self.new_map[i]] = output_array[i]

            for i in range(self.input_dimension, self.new_input_dimension):
                # print("i: ", i)
                # print("new map", self.map[i])
                # print("input array", input_array)
                new_output_array[self.map[i]] |= input_array[i]
